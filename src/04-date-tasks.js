...
function timeSpanToString(startDate, endDate) {
  ...
  const secondStart = startDate.getSeconds();
  const secondEnd = endDate.getSeconds();
  const milliSecondStart = startDate.getMilliseconds();
  const milliSecondEnd = endDate.getMilliseconds();

  let hour = hourEnd - hourStart;
  hour = hour < 10 ? `0${hour}` : hour;
  let minute = minuteEnd - minuteStart;
  minute = minute < 10 ? `0${minute}` : minute;
  let second = secondEnd - secondStart;
  second = second < 10 ? `0${second}` : second;
  let milliSecond = milliSecondEnd - milliSecondStart;
...
function angleBetweenClockHands(date) {
  ...
  const uHour = 0.5 * (60 * hour + minute);
  const uMinute = 6 * minute;
  let u = uHour - uMinute;
  u = (u > 180) ? 360 - u : u;
  u = (u < 0) ? u * -1 : u;
